...
# Configurația pool-ului de conexiuni cu timeout-uri mai scurte
db_pool = pooling.MySQLConnectionPool(
    pool_name="rezervari_pool_v5",
    pool_size=20,  # Redus de la 32 la 20
    ...
    database="rezervari",
    autocommit=False,
    # Adăugăm timeout-uri pentru a evita conexiunile blocate
    connection_timeout=10,
    # Configurații suplimentare pentru cleanup
    sql_mode='TRADITIONAL'
)

@contextmanager
def get_db_connection():
    """Context manager pentru gestionarea sigură a conexiunilor"""
    cnx = None
    try:
...
def get_db_cursor(cnx, dictionary=True):
    """Context manager pentru gestionarea sigură a cursor-urilor"""
    cursor = None
    try:
...
def get_recommendations(room_groups, requested_adults, requested_rooms_count):
    ...
    # Începem prin a verifica dacă o singură cameră este suficientă
    single_room_options = []
    for room in all_available_rooms:
        if room['capacity'] >= requested_adults:
            single_room_options.append([room])  # Adăugăm ca o listă pentru a păstra formatul

    # Dacă am găsit camere individuale potrivite, folosim DOAR acele opțiuni
    if single_room_options:
        valid_combinations = single_room_options
    else:
        # Altfel, dacă nicio cameră individuală nu este suficientă, căutăm combinații
        valid_combinations = []
        max_rooms_to_check = requested_rooms_count + 1
        # Începem căutarea de la 2 camere în sus
    ...
    for combo in valid_combinations:
        # Folosim un frozenset de nume de camere pentru a identifica combinațiile unice ca structură
        combo_key = frozenset(room['name'] for room in combo)
        
        # Cheia procesată va fi o tuplă de (număr de camere, setul de nume de camere)
        processed_key = (len(combo), combo_key)
        if processed_key in processed_combos:
            continue
        ...
        # Criterii de sortare:
        # 1. Prioritizează numărul de camere cerut (dacă e posibil)
        # 2. Minimul de capacitate în plus
        # 3. Prețul total cel mai mic
        ...
        summary = {}
        # Creăm o copie temporară a ID-urilor de camere disponibile
        temp_room_ids = {group['name']: group['room_ids'].split(',') for group in room_groups if group.get('room_ids')}
        
        # Iterăm prin camerele din combinația curentă pentru a aloca ID-uri și a crea sumarul
        package_room_ids = []
        ...
        for room in combo:
            ...
            if temp_room_ids.get(room_name) and temp_room_ids[room_name]:
                ...
                # Dacă nu avem un ID de cameră disponibil, pachetul nu este valid
...
def register():
    if request.method == 'POST':
        ...
        try:
            with get_db_connection() as cnx:
                with get_db_cursor(cnx) as cursor:
                    # Corectat: Căutăm după 'user_id' (deși aici e ok și cu 'id' sau orice altceva)
                    ...
                    # Corectat: Inserăm in coloanele corecte. 'is_admin' are valoare default 0 în DB.
            ...
            with get_db_connection() as cnx:
                with get_db_cursor(cnx) as cursor:
                    ...
                    if user and bcrypt.checkpw(password.encode('utf-8'), user['password'].encode('utf-8')):
                        # Corectat: Folosim 'user_id' și 'is_admin' conform schemei DB
                        session['user_id'] = user['user_id'] 
                        session['username'] = user['username']
                        session['is_admin'] = user.get('is_admin', 0) # 0 pentru non-admin
...
@app.route('/profile')
def profile():
    # Se verifică dacă utilizatorul este logat folosind funcția ajutătoare
    if not is_logged_in():
        return redirect(url_for('login'))
    
    # Dacă utilizatorul este admin, este redirecționat către panoul de administrare
    if session.get('is_admin'):
        return redirect(url_for('admin'))

    try:
        # Se folosește connection pooling și un context manager pentru siguranță
        with get_db_connection() as cnx:
            with get_db_cursor(cnx) as cursor:
                user_id = session['user_id']
                
                # 1. Preluarea datelor utilizatorului curent
                cursor.execute("SELECT * FROM users WHERE user_id = %s", (user_id,))
                user = cursor.fetchone()

                # 2. Preluarea proprietăților deținute de utilizator
                cursor.execute("SELECT * FROM properties WHERE owner_id = %s", (user_id,))
                proprietati = cursor.fetchall()
                
                # 3. Preluarea rezervărilor făcute de utilizator, cu detalii despre proprietate
                cursor.execute("""
                    SELECT r.*, p.name as property_name, p.city
                    FROM reservations r
                ...
                return render_template('profile.html', user=user, rezervari=rezervari, proprietati=proprietati, logged_in=True)
...
def edit_profile():
    # Verificare consistentă a sesiunii
    ...
    try:
        with get_db_connection() as cnx:
            with get_db_cursor(cnx) as cursor:
                # Logica pentru trimiterea formularului (POST)
                if request.method == 'POST':
                    ...
                    # Pas 1: Verificarea identității cu parola curentă
                    ...
                    # Pas 2: Validarea unicității datelor noi
                    ...
                    # Construirea și executarea comenzii UPDATE
                    ...
                    # Pas 4: Actualizarea sesiunii și feedback
    ...
    try:
        with get_db_connection() as cnx:
            with get_db_cursor(cnx) as cursor:
                ...
                return render_template("admin.html", flask_status="Online", iis_status="Offline (not implemented)", total_properties=total_properties, total_users=total_users, active_sessions_count=len(session), active_reservations=active_reservations, cancelled_reservations=cancelled_reservations, ocupate=ocupate, disponibile=disponibile, total_rooms_count=total_rooms_count, period=period, start_date=start_date, end_date=end_date, logged_in=True)
    except Exception as e:
            ...
            with get_db_cursor(cnx) as cursor:
                ...
                if request.method == 'POST':
                    try:
                        ...
                        for name in set(existing_room_types_db.keys()) - processed_room_types:
                            cursor.execute("SELECT 1 FROM reservations res JOIN rooms r ON res.room_id = r.idrooms WHERE r.property_id = %s AND r.name = %s LIMIT 1", (property_id, name))
                            if cursor.fetchone(): raise ValueError(f"Tipul de cameră '{name}' are un istoric de rezervări și nu poate fi șters.")
...
def cancel_reservation(reservation_id):
    ...
    try:
        with get_db_connection() as cnx: # Corectat aici
            with cnx.cursor(dictionary=True) as cursor:
                cursor.execute("""
                    SELECT r.user_id, r.status, rm.property_id 
                    FROM reservations r 
                ...
                if not res_info: 
                    raise ValueError("Rezervarea nu există.")
                
                cursor.execute("""
                    SELECT owner_id 
                    FROM properties 
                    WHERE property_id = %s
                """, (res_info['property_id'],))
                owner = cursor.fetchone()
                
                is_owner = owner and owner['owner_id'] == session['user_id']
                
                if not (res_info['user_id'] == session['user_id'] or is_owner): 
                    raise ValueError("Operațiune nepermisă.")
...
def booking_confirmation():
    ...
    try:
        # AICI SE CREEAZĂ OBIECTELE DATE REALE
        start_date = datetime.strptime(start_str, "%Y-%m-%d").date()
        end_date = datetime.strptime(end_str, "%Y-%m-%d").date()
        nights = (end_date - start_date).days
        
        if nights <= 0: 
            return "Perioadă invalidă.", 400
            ...
            with cnx.cursor(dictionary=True) as cursor:
                if request.method == 'POST':
                    ...
                    try:
                        ...
                        cursor.execute(f"""
                            SELECT r.idrooms, r.price 
                            FROM rooms r 
                            WHERE r.idrooms IN ({format_strings})
                        """, tuple(room_id_list))
                        package_rooms_db = cursor.fetchall()
                        
                        if len(package_rooms_db) != len(room_id_list): 
                            raise ValueError("Camerele nu mai sunt disponibile.")
                        ...
                        for room in package_rooms_db:
                            ...
                            if not first_res_id: 
                                first_res_id = cursor.lastrowid
                        
                        if first_res_id:
                            cursor.execute("""
                                INSERT INTO reservation_details (reservation_id, full_name, phone, email, total_price) 
                                VALUES (%s, %s, %s, %s, %s)
                            """, (first_res_id, request.form['full_name'], 
                                 request.form['phone'], request.form['email'], total_price))
                ...
                # GET request - Afișarea formularului de confirmare
                format_strings = ','.join(['%s'] * len(room_id_list))
                cursor.execute(f"""
                ...
                # ****** LINIA CRITICĂ ESTE AICI ******
                # Trimitem către HTML obiectele 'start_date' și 'end_date', nu string-urile.
                return render_template("booking_confirmation.html", 
                                     package_rooms=package_rooms, 
                                     start=start_date, # TREBUIE SĂ FIE start_date
                                     end=end_date,     # TREBUIE SĂ FIE end_date
                                     total_price=total_price, 
                                     nights=nights, 
                                     logged_in=True)
                                     
    except Exception as e:
        flash(f"A apărut o eroare la confirmarea rezervării: {e}", "error")
        return redirect(url_for('home'))
...
def booking_success():
    ...
    try:
        with get_db_connection() as cnx:
            with cnx.cursor(dictionary=True) as cursor:
                # Interogarea SQL a fost corectată pentru a include det.total_price
                cursor.execute("""
                    SELECT res.*, r.name AS room_name, p.name AS property_name, 
                           DATEDIFF(res.end_date, res.start_date) AS nights, 
                           det.full_name, det.email, det.total_price 
    ...
    try:
        with get_db_connection() as cnx: # Corectat aici
            with cnx.cursor(dictionary=True) as cursor:
                ...
                return render_template('manage_property_reservations.html', 
                                     prop=prop, 
                                     reservations=reservations, 
                                     logged_in=True)
...
def check_room_type_reservations(property_id, room_type_name_encoded):
    ...
    if 'user_id' not in session: 
        return jsonify({"has_reservations": False}), 401
        ...
        with get_db_connection() as cnx: # Corectat aici
            with cnx.cursor(dictionary=True) as cursor:
                ...
                if not prop_owner or prop_owner['owner_id'] != session['user_id']:
                    return jsonify({"has_reservations": False}), 403
                ...
                return jsonify({"has_reservations": active_reservations_of_type > 0})
                
    except MySQLError as e:
        return jsonify({"has_reservations": False, "error": str(e)}), 500
