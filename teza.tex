...
În contextul aplicației Flask de rezervări, limbajul SQL este utilizat ca principal instrument pentru interacțiunea cu baza de date MySQL. Operațiile efectuate în backend, precum înregistrarea unui utilizator, autentificarea, crearea sau anularea unei rezervări sunt transpuse în comenzi SQL care sunt transmise serverului de baze de date pentru execuție. SQL oferă o interfață unificată pentru aceste acțiuni, indiferent de complexitatea logicii aplicației, facilitând astfel gestionarea coerentă a datelor persistente. Prin folosirea unui limbaj standardizat, aplicația poate menține portabilitatea și compatibilitatea cu alte sisteme bazate pe același model relațional.
...
\section{Nginx (Load Balancing și Reverse Proxy)}
Nginx (Engine-X) \cite{ng}  este un server web de înaltă performanță, un reverse proxy și un load balancer. A fost ales pentru a îndeplini acest rol în cadrul arhitecturii noastre, servind atât ca server web, cât și ca reverse proxy cu capabilități de load balancing. Această componentă este poziționată strategic în fața serverelor de aplicații Flask, acționând ca un "distribuitor de trafic" și un punct de intrare unic pentru toate cererile utilizatorilor.
\subsection{Principiul de Funcționare al Load Balancing-ului}
Load balancing-ul este procesul de distribuire a traficului de rețea sau a sarcinilor de lucru între mai multe servere. Scopul principal este de a optimiza utilizarea resurselor, de a maximiza debitul, de a minimiza timpul de răspuns și de a evita supraîncărcarea unui singur server. Fără un load balancer, un vârf de trafic ar putea copleși un singur server, ducând la încetinirea aplicației sau chiar la căderea acesteia. Nginx, configurat ca load balancer, primește toate cererile HTTP/HTTPS de la clienți și le direcționează inteligent către una dintre multiplele instanțe de aplicație Flask (numite upstreams în terminologia Nginx). Există diverse strategii (algoritmi) de load balancing pe care Nginx le poate implementa:
\begin{itemize}
	...
	\item IP Hash: Direcționează cererile pe baza adresei IP a clientului. Aceasta asigură că un client anume va fi întotdeauna trimis către același server, util pentru aplicațiile care depind de persistența sesiunii pe server. Beneficiile cheie ale acestei abordări sunt multiple:
	\begin{enumerate}
		\item Îmbunătățirea performanței: Prin distribuirea uniformă a sarcinii, se reduce timpul de răspuns per cerere și se crește capacitatea totală de prelucrare a cererilor.
		\item Scalabilitate: Permite scalarea orizontală a aplicației prin adăugarea sau eliminarea dinamică a instanțelor Flask fără întreruperea serviciului. Pe măsură ce traficul crește, pot fi lansate noi instanțe Flask și adăugate în grupul de upstreams al Nginx.
		\item Toleranță la erori (Failover): Dacă una dintre instanțele Flask devine indisponibilă, Nginx o detectează automat și o exclude temporar din rotație, direcționând tot traficul către celelalte instanțe sănătoase. Acest lucru asigură o înaltă disponibilitate și o reziliență sporită a întregului sistem.
	\end{enumerate}	
\end{itemize}
\subsection{Integrarea Nginx cu Flask ca Reverse Proxy}
Pe lângă rolul său de load balancer, Nginx acționează și ca un reverse proxy pentru aplicația Flask. Un reverse proxy este un server care preia cereri de la clienți și le redirecționează către servere interne, ascunzând complexitatea rețelei interne de client. Configurația Nginx implică definirea unui bloc upstream care listează adresele (IP și port) ale tuturor instanțelor Flask disponibile. Apoi, într-un bloc server care ascultă pe porturile HTTP/HTTPS standard (80/443), cererile sunt redirecționate către grupul upstream folosind directive precum proxy\_pass. Un alt aspect vital al integrării Nginx este capacitatea sa de a servi fișiere statice (CSS, JavaScript, imagini încărcate de utilizatori). Aplicațiile Flask pot servi fișiere statice, dar nu sunt optimizate pentru acest lucru, iar a face acest lucru direct ar consuma resurse prețioase ale serverului de aplicații. Nginx este extrem de eficient în servirea fișierelor statice direct din sistemul de fișiere, reducând semnificativ sarcina pe serverul Flask. De exemplu, cererile către /static/ sau /uploads/ pot fi configurate în Nginx pentru a fi servite direct, în timp ce toate celelalte cereri dinamice sunt transmise către Flask. 
...
Rolul Nginx ca load balancer și reverse proxy are un impact pozitiv semnificativ asupra tuturor fluxurilor de aplicație descrise anterior, contribuind la performanță, scalabilitate și reziliență:
\begin{itemize}
	\item Înregistrare și Autentificare: Chiar și în condiții de trafic intens, cererile de înregistrare și autentificare sunt distribuite, asigurând că procesul de creare de conturi noi și de accesare a conturilor existente rămâne rapid și responsiv. Dacă un server Flask ar ceda în timpul unui vârf de trafic, Nginx ar redirecționa automat noile cereri către serverele rămase, minimizând disrupția.
	
	\item Căutare și Vizualizare Proprietăți: Fluxul de căutare, care poate genera un volum mare de cereri SELECT către backend, beneficiază direct de distribuția încărcării. Utilizatorii experimentează timpi de răspuns rapizi, indiferent de numărul de cereri concurente. De asemenea, servirea imaginilor proprietăților direct de către Nginx (care este mult mai eficient decât Flask pentru acest lucru) reduce sarcina pe serverele Flask și îmbunătățește viteza de încărcare a paginilor.
	
	\item Crearea unei Rezervări: Chiar dacă procesul de rezervare este complex și tranzacțional, distribuirea cererilor inițiale (de la formularul de căutare sau de vizualizare proprietate) către servere diferite asigură că sistemul rămâne receptiv. Nginx ajută la menținerea unei conexiuni stabile cu serverul Flask care procesează rezervarea.
	\item Fluxurile de Management (Proprietari și Admin): Deși aceste fluxuri pot avea un volum mai mic de trafic, ele beneficiază de aceeași reziliență. Administratorii și proprietarii pot gestiona proprietățile și rezervările fără întârzieri, chiar dacă alte părți ale sistemului sunt sub presiune sau dacă o instanță de server are probleme. În esență, Nginx acționează ca un scut protector și un optimizator, asigurând că aplicația este întotdeauna disponibilă, rapidă și capabilă să scaleze pentru a răspunde cerințelor utilizatorilor.
...
Creșterea exponențială a numărului de utilizatori și a volumului de date în mediul online a impus noi provocări în dezvoltarea aplicațiilor web. Sistemele software moderne, în special platformele care intermediază servicii, trebuie să gestioneze un număr ridicat de interacțiuni simultane fără a compromite performanța, securitatea și fiabilitatea. Tranziția procesului de rezervare din mediul tradițional către platforme digitale, accelerată de omniprezența dispozitivelor conectate la internet, impune constrângeri tehnice semnificative, transformând gestiunea cererilor concurente și a volumelor mari de date în provocări centrale.

În acest context, lucrarea de față propune studiul, proiectarea și implementarea unei platforme web pentru rezervări de cazare, servind drept studiu de caz pentru aplicarea unor concepte avansate de inginerie software. Obiectivul fundamental depășește crearea unei aplicații exclusiv funcționale, vizând construirea unui sistem a cărui arhitectură să fie, prin proiectare, performantă și robustă. Pentru a atinge aceste deziderate, elaborarea proiectului a fost ghidată de un set de obiective tehnice precise:
\begin{enumerate}
	...
	\item \textbf{Testabilitate:} Realizarea componentelor software într-o formă care permite aplicarea eficientă a testelor unitare și de integrare, contribuind la menținerea unui comportament previzibil și fiabil al aplicației.
\end{enumerate}



\section{Arhitectura Generală a Sistemului Distribuit}
Soluția arhitecturală propusă pentru îndeplinirea acestor obiective este una distribuită, multi-nod. Sistemul nu se bazează pe o singură mașină, ci pe un ansamblu de componente interconectate, fiecare cu un rol specializat. Această structură, prezentată vizual în Figura \ref{fig:mod}, este fundamentul strategiei de disponibilitate și echilibrare a încărcării.
...
\begin{enumerate}
\item Stația de Lucru Principală (Host): Aceasta reprezintă mediul de lucru principal unde au fost dezvoltate și găzduite o parte dintre componentele esențiale ale aplicației. Include o instanță a aplicației Python Flask, care servește ca logică de business, și serverul MySQL Master, responsabil pentru operațiile de scriere în baza de date. De asemenea, Nginx Load Balancer este configurat pe această stație, gestionând și distribuind traficul HTTP către celelalte instanțe Flask.
\item Mașină Virtuală 1 (VM1 - Mediu Testare/DB Slave): Această mașină virtuală este dedicată mediului de testare și găzduiește serverul MySQL Slave. Rolul său principal este de a prelua și executa operațiile de citire din baza de date și de a asigura redundanța datelor prin replicarea modificărilor de la serverul Master. Astfel, optimizarea operațiilor de citire-scriere este un obiectiv central al acestei configurații.
\item Mașină Virtuală 2 (VM2 - Server Secundar): Această mașină virtuală găzduiește o instanță secundară a aplicației Flask. Rolul său este de a prelua cererile distribuite de Nginx, contribuind la creșterea performanței și puterii de procesare a aplicației web prin load balancing. Această instanță, alături de cea de pe Host, asigură scalabilitatea orizontală a stratului de aplicație.
\item Nginx Load Balancer: Poziționat pe stația de lucru principală, Nginx acționează ca un reverse proxy și distribuitor de trafic. El primește toate cererile de la utilizatori și le direcționează inteligent către instanțele Flask disponibile pe Host și VM2, conform strategiilor de load balancing. De asemenea, Nginx este optimizat pentru a servi fișierele statice, reducând sarcina pe serverele Flask.
\item Aplicație Python Flask: Aceasta reprezintă stratul de logică de business al aplicației web, rulând pe Host și VM2. Se ocupă cu procesarea cererilor utilizatorilor, interacțiunea cu baza de date (MySQL Master și Slave) și gestionarea sesiunilor și autentificării.
\item MySQL Database (Master/Slave): Configurația de bază de date este de tip master-slave, unde serverul Master (pe Host) gestionează toate scrierile, iar serverul Slave (pe VM1) se ocupă de citiri. Această separare a sarcinilor optimizează performanța operațiilor de citire și scriere și asigură înalta disponibilitate a datelor prin replicare.
...
Baza de date a aplicației "rezervari" este concepută pentru a stoca și gestiona eficient toate informațiile legate de utilizatori, proprietăți, camere, rezervări și plățile aferente. Structura sa relațională asigură integritatea datelor și permite interogări complexe necesare funcționalităților aplicației. Baza de date este compusă din șase tabele principale, interconectate prin chei primare și externe, asigurând integritatea referențială și organizarea logică a datelor.



\subsubsection{Tabela \texttt{users}}
\begin{itemize}
	\item \textbf{Scop:} Tabela \texttt{users} este fundamentul aplicației, stocând toate informațiile esențiale despre utilizatorii care interacționează cu platforma. Aceasta include detalii necesare pentru autentificare, autorizare și personalizarea experienței.
	...
	\begin{itemize}
		\item \textbf{`users` (1) --- `properties` (M):} Un utilizator poate deține zero sau mai multe proprietăți. Relația este stabilită prin cheia externă \texttt{owner\_id} din tabela \texttt{properties}, care referă la \texttt{user\_id} din \texttt{users}. Aceasta permite aplicației să afișeze proprietățile unui anumit proprietar.
		\item \textbf{`users` (1) --- `reservations` (M):} Un utilizator poate efectua zero sau mai multe rezervări. Această legătură este realizată prin cheia externă \texttt{user\_id} din tabela \texttt{reservations}, care referă la \texttt{user\_id} din \texttt{users}. Este esențială pentru a urmări istoricul rezervărilor unui client.
	\end{itemize}
\end{itemize}

\subsubsection{Tabela \texttt{properties}}
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \texttt{owner\_id}: O cheie externă crucială care face legătura cu tabela \texttt{users}, specificând cine este proprietarul acestei proprietăți.
	...
	\item \textbf{Relații și implicații:}
	\begin{itemize}
		\item \textbf{`properties` (1) --- `rooms` (M):} O proprietate poate conține una sau mai multe camere (sau tipuri de camere). Această legătură este realizată prin cheia externă \texttt{property\_id} din tabela \texttt{rooms}, care referă la \texttt{property\_id} din \texttt{properties}. Aceasta permite organizarea logică a spațiilor de cazare sub umbrela fiecărei proprietăți.
...
\subsubsection{Tabela \texttt{rooms}}
\begin{itemize}
	\item \textbf{Scop:} Tabela \texttt{rooms} este granulară, stocând detalii specifice despre fiecare instanță de cameră (sau tip de cameră, în funcție de interpretarea coloanei \texttt{number\_of}) disponibilă pentru rezervare.
	\item \textbf{Coloane cheie și semnificația lor:}
	\begin{itemize}
		\item \texttt{idrooms}: Cheia primară auto-incrementată pentru fiecare cameră înregistrată.
		\item \texttt{property\_id}: Cheia externă către proprietatea căreia îi aparține camera.
		...
		\item \texttt{available}: Un flag boolean care indică disponibilitatea generală a camerei (dacă poate fi rezervată sau nu, independent de perioadă).
	\end{itemize}
	\item \textbf{Relații și implicații:}
	\begin{itemize}
		\item \textbf{`rooms` (1) --- `reservations` (M):} O cameră poate fi inclusă în zero sau mai multe rezervări de-a lungul timpului. Această legătură se face prin cheia externă \texttt{room\_id} din tabela \texttt{reservations}. Este fundamentală pentru a urmări ce cameră a fost rezervată.
		\item Relația cu \texttt{properties} asigură că ștergerea unei proprietăți duce la ștergerea în cascadă a camerelor asociate (`ON DELETE CASCADE`).
	\end{itemize}
\end{itemize}

\subsubsection{Tabela \texttt{reservations}}
\begin{itemize}
	\item \textbf{Scop:} Tabela \texttt{reservations} este nucleul sistemului de rezervări, înregistrând fiecare tranzacție de rezervare în parte.
	\item \textbf{Coloane cheie și semnificația lor:}
	\begin{itemize}
		\item \texttt{reservation\_id}: Cheia primară auto-incrementată pentru identificarea unică a fiecărei rezervări.
		\item \texttt{user\_id}: Cheia externă care indică utilizatorul care a inițiat rezervarea.
		\item \texttt{room\_id}: Cheia externă către camera specifică care a fost rezervată.
	...
	\item \textbf{Relații și implicații:}
	\begin{itemize}
		\item \textbf{`reservations` (1) --- `reservation\_details` (1):} O rezervare are un singur set de detalii de contact suplimentare. Legătura este prin cheia externă \texttt{reservation\_id} din \texttt{reservation\_details}.
		\item \textbf{`reservations` (1) --- `payments` (M):} O rezervare poate avea zero sau mai multe plăți asociate (util pentru scenarii de plăți parțiale, reîncercări de plată sau eșecuri). Legătura se face prin cheia externă \texttt{reservation\_id} din tabela \texttt{payments}.
...
\subsubsection{Tabela \texttt{reservation\_details}}
\begin{itemize}
	\item \textbf{Scop:} Această tabelă este dedicată stocării detaliilor de contact specifice clientului care a efectuat rezervarea, precum și a prețului total al acesteia. Prin separarea acestor informații, se respectă principiile de normalizare, evitându-se redundanța și facilitându-se gestionarea datelor sensibile.
	\item \textbf{Coloane cheie și semnificația lor:}
	\begin{itemize}
		\item \texttt{detail\_id}: Cheia primară auto-incrementată pentru fiecare intrare de detalii.
		\item \texttt{reservation\_id}: Cheia externă care leagă direct aceste detalii la o înregistrare specifică din tabela \texttt{reservations}.
...
Logica aplicației se traduce în interacțiuni specifice cu această structură de baze de date, gestionând ciclul de viață al unei rezervări:
...
\begin{enumerate}
	...
	\item \textbf{Căutarea Proprietăților:} Când un client caută o proprietate, aplicația interoghează tabelele \texttt{properties} și \texttt{rooms} (posibil și \texttt{users} pentru numele proprietarului) pentru a găsi proprietățile care corespund criteriilor și a verifica disponibilitatea camerelor pentru o anumită perioadă. Acestea sunt în principal operații de citire.
	...
	\begin{itemize}
		...
		\item Toate aceste operațiuni sunt realizate în cadrul unei tranzacții ACID, asigurând că fie toate înregistrările sunt create cu succes, fie niciuna nu este. Acest lucru previne stările inconsistente ale datelor (ex: rezervare creată fără detalii de contact).
	\end{itemize}
...
Utilizarea cheilor externe (\texttt{FOREIGN KEY}) cu reguli precum \texttt{ON DELETE CASCADE} și \texttt{ON UPDATE CASCADE} (acolo unde sunt definite) este esențială pentru a menține integritatea referențială a bazei de date. De exemplu, dacă un utilizator este șters din tabela \texttt{users}, toate proprietățile și rezervările asociate acestuia vor fi șterse automat din tabelele \texttt{properties} și \texttt{reservations}. Acest mecanism reduce riscul de "date orfane" și simplifică logica aplicației prin delegarea managementului consistenței către sistemul de gestiune a bazelor de date. În cazurile în care \texttt{FOREIGN KEY} nu este aplicată la nivel de schemă (cum ar fi relația \texttt{reservation\_details} - \texttt{reservations}), aplicația este responsabilă să asigure manual integritatea datelor.
Dacă arhitectura generală descrie interacțiunea la nivel de sistem, arhitectura internă detaliază modul în care este construită componenta centrală a acestuia: aplicația web. Pentru implementarea acestui nucleu s-a utilizat framework-ul Flask, o alegere motivată de abordarea sa minimalistă și flexibilă.
...
Organizarea codului sursă este esențială pentru mentenanță. Proiectul adoptă o structură clară, conform principiului separării responsabilităților, așa cum este ilustrat în Figura \ref{fig:struc_pro}. Fișierul \texttt{app.py} reprezintă punctul de intrare și nucleul logic al aplicației; aici se inițializează instanța Flask, se configurează conexiunea la baza de date, se definesc rutele, funcțiile de suport și logica de business. Directorul \texttt{templates/} conține toate fișierele HTML care alcătuiesc interfața vizuală, realizându-se astfel o separare eficientă a logicii de prezentare.

\begin{figure}[H]
	\centering
	\includegraphics*[width=1\columnwidth, center]{figuri/strucpr.png}
	\caption{Structura ierarhică a proiectului Flask}
	\label{fig:struc_pro}
\end{figure}
\subsection{Biblioteci Principale Utilizate}
Funcționalitatea aplicației este susținută de un set de biblioteci Python, fiecare având un rol bine definit, așa cum este evidențiat în Figura \ref{fig:bib}.

\begin{figure}[H]
	\centering
	\includegraphics*[width=1\columnwidth, center]{figuri/bib.png}
	\caption{Biblioteci principale utilizate în proiect}
	
	\label{fig:bib}
\end{figure}

Rolul fiecărei biblioteci principale este detaliat mai jos:
\begin{itemize}
	\item \textbf{flask:} Aceasta este biblioteca fundamentală. Din ea se importă obiecte esențiale:
	\begin{itemize}
		\item \textit{Flask:} Clasa principală utilizată pentru crearea instanței aplicației.
		\item \textit{render\_template:} Funcția cheie pentru a invoca motorul de șabloane Jinja2 și a genera pagini HTML dinamice.
		\item \textit{request:} Un obiect global care încapsulează toate informațiile despre cererea HTTP curentă (metodă, date de formular, parametri URL, antete).
		\item \textit{redirect, url\_for:} Unelte pentru a gestiona fluxul de navigare, permițând redirecționarea utilizatorului către alte rute.
		\item \textit{session:} Un obiect de tip dicționar utilizat pentru a stoca date specifice unui utilizator între cereri (ex., ID-ul utilizatorului după logare), securizat printr-un cookie semnat criptografic.
		\item \textit{flash:} Un mecanism pentru a afișa mesaje temporare utilizatorului (ex., "Înregistrare reușită!").
		\item \textit{jsonify:} O funcție utilizată pentru a crea răspunsuri HTTP cu formatul application/json.
	\end{itemize}
	
	\item \textbf{mysql.connector:} Biblioteca oficială pentru conectarea la MySQL. S-a utilizat sub-modulul \textit{pooling} pentru a implementare \textit{Connection Pooling}, o tehnică de optimizare ce menține un set de conexiuni deschise și refolosibile.
	
	\item \textbf{bcrypt:} O bibliotecă esențială pentru securitate, dedicată procesării (hashing) parolelor. Utilizează algoritmul Blowfish și mecanismul de "salting" pentru a proteja împotriva atacurilor de tip forță brută și "rainbow table".
	
	\item \textbf{biblioteci standard Python:}
	\begin{itemize}
		\item \textit{datetime, date, timedelta:} Folosite intensiv pentru a manipula și calcula intervale de timp, necesare pentru gestionarea perioadelor de rezervare.
		\item \textit{collections.defaultdict:} O structură de date avansată utilizată în algoritmul de calcul al zilelor blocate.
		\item \textit{itertools:} O bibliotecă puternică pentru crearea de iteratori eficienți, utilizată în motorul de recomandări.
	\end{itemize}
\end{itemize}
\subsection{Configurarea Aplicației}
Configurarea critică a instanței aplicației include definirea \texttt{app.secret\_key}. Această cheie secretă este utilizată de Flask pentru a semna criptografic cookie-urile de sesiune, un mecanism vital pentru a preveni manipularea datelor de sesiune de către client și pentru a securiza sistemul de autentificare.
...
Funcționalitățile de hashing sunt oferite de biblioteca \textbf{bcrypt}, o soluție modernă și extrem de sigură, recunoscută ca un standard de facto pentru securizarea parolelor. Spre deosebire de funcțiile de hash rapide precum \texttt{SHA-256} (care sunt create pentru viteză și sunt inadecvate pentru parole), \textbf{bcrypt} este un algoritm \textbf{adaptiv}, proiectat special pentru a fi încet și costisitor din punct de vedere computațional. Această caracteristică, numită "work factor", poate fi ajustată pentru a contracara creșterea puterii de calcul a hardware-ului modern, asigurând astfel relevanța sa pe termen lung.
...
\paragraph{Procesul de Înregistrare}
...
\paragraph{Procesul de autentificare}
...
\paragraph{Crearea unui Utilizator Nou (Operațiunea \texttt{INSERT})}
...
\paragraph{Regăsirea Datelor la Autentificare (Operațiunea \texttt{SELECT})}
...
\paragraph{Logica Backend (Ruta \texttt{/edit-profile} în \texttt{app.py})}
...
\section{Panoul de Administrare: Privilegiu și Agregare de Date}
...
\subsection{Diferența Fundamentală față de Profilul de Utilizator}
...
\paragraph{Logica Backend (Rutele \texttt{/edit\_property} și \texttt{/check\_room\_type...})}
...
\paragraph{Interfața Frontend (\texttt{my\_properties.html})}
...
\paragraph{Logica Backend (Ruta \texttt{/delete\_property/<id>} în \texttt{app.py})}
...
\subsubsection{Prezentarea Frontend (\texttt{search\_results.html} și \texttt{view\_property.html})}
...
\subsubsection{Logica Backend (\texttt{/property/<id>} în \texttt{app.py})}
...
\subsection{Confirmarea și Finalizarea Rezervării (\texttt{/booking/confirm})}
...
\subsubsection{Prezentarea Frontend (\texttt{booking\_confirmation.html})}
...
\subsubsection{Logica Backend (\texttt{/booking/confirm} în \texttt{app.py})}
...
\subsection{Pagina de Confirmare a Rezervării (\texttt{/booking/success})}
...
\subsubsection{Prezentarea Frontend (\texttt{reservation\_success.html})}
...
\subsubsection{Logica Backend (\texttt{/booking/success} în \texttt{app.py})}
...
\subsection{Vizualizarea și Gestionarea Rezervărilor}
...
\subsubsection{Prezentarea Frontend (\texttt{my\_reservations.html})}
...
\subsubsection{Logica Backend (\texttt{/my-reservations} în \texttt{app.py})}
...
\subsection{Anularea Rezervării (\texttt{/cancel\_reservation/<int:reservation\_id>})}
...
\subsubsection{Logica Backend (\texttt{/cancel\_reservation/<int:reservation\_id>} în \texttt{app.py})}
\begin{enumerate}
...
\subsection{Gestionarea Rezervărilor Proprietăților Personale}
...
\subsubsection{Prezentarea Frontend (\texttt{manage\_property\_reservations.html})}
...
\subsubsection{Logica Backend (\texttt{/manage\_property\_reservations/<int:property\_id>} în \texttt{app.py})}
...
Acest capitol se concentrează pe elementele fundamentale ale infrastructurii și arhitecturii de suport care asigură scalabilitatea, reziliența și performanța necesare unei aplicații web moderne, capabile să gestioneze un trafic considerabil și să funcționeze fără întreruperi. Dincolo de logica de afaceri a aplicației, modul în care aceasta este găzduită și gestionată la nivel de infrastructură este la fel de crucial pentru succesul său. Se vor explora mecanisme cheie precum \texttt{load balancing}-ul, replicarea bazelor de date și rolul virtualizării în testarea și implementarea unei astfel de arhitecturi. Înțelegerea acestor concepte este vitală pentru a construi un sistem care nu doar funcționează, ci funcționează fiabil și eficient la scară.

\section{Rolul Nginx (Load Balancing și Reverse Proxy) în Contextul Arhitecturii Implementate}
Așa cum a fost detaliat în Secțiunea 2.5, Nginx joacă un rol pivotal în asigurarea scalabilității și rezilienței aplicației web prin funcționalitățile sale de load balancing și reverse proxy. În cadrul arhitecturii implementate pentru sistemul de rezervări, Nginx este configurat pentru a gestiona eficient traficul către instanțele aplicației Flask, contribuind direct la optimizarea performanței și disponibilității sistemului.



\subsection{Aplicarea Load Balancing-ului în Arhitectura Proiectului}
În contextul arhitecturii noastre distribuite, Nginx, configurat pe stația de lucru principală (Host), acționează ca un punct de intrare unic pentru toate cererile utilizatorilor. Aceste cereri sunt apoi distribuite între cele două instanțe ale aplicației Flask: una locală (pe Host) și una găzduită pe Mașina Virtuală 2 (VM2). Strategia de distribuție asigură o repartizare echitabilă a încărcării, prevenind supraîncărcarea unei singure instanțe Flask. Această distribuție activă a traficului este esențială pentru a gestiona volume mari de cereri concurente și pentru a menține timpi de răspuns rapizi, chiar și în condiții de trafic intens. În plus, capacitatea Nginx de a detecta instanțele Flask indisponibile și de a redirecționa traficul către cele funcționale contribuie semnificativ la reziliența și înalta disponibilitate a stratului de aplicație.
...
\section{Mecanismul Remarcabil: Rolul Replicării MySQL (Master-Slave) în Contextul Fluxurilor Reale}
Scalabilitatea și înalta disponibilitate a stratului de bază de date sunt critice pentru performanța și reziliența sistemului de rezervări, mai ales având în vedere natura predominant "read-heavy" a aplicațiilor web de acest tip. Pentru a răspunde acestor cerințe, în arhitectura noastră a fost implementat un mecanism robust de replicare MySQL de tip Master-Slave, cu nodul Master găzduit pe stația de lucru principală (Host) și nodul Slave pe Mașina Virtuală 1 (VM1). Această configurație permite o gestionare optimizată a operațiilor de citire și scriere, precum și o strategie eficientă de continuitate a serviciului în caz de defecțiune.

\subsection{Separarea Citirilor de Scrieri (Read/Write Splitting) în Implementarea Proiectului}
Unul dintre beneficiile majore ale configurației Master-Slave, esențial pentru optimizarea performanței, este posibilitatea de a separa cererile de citire (SELECT) de cele de scriere (INSERT, UPDATE, DELETE). În cadrul proiectului, nodul MySQL Master (pe Host) este configurat să gestioneze exclusiv toate operațiile de scriere, asigurând consistența și integritatea datelor. Concomitent, nodul MySQL Slave (pe VM1) preia majoritatea operațiilor de citire. Această distribuție a sarcinilor maximizează debitul și reduce contestația pe nodul Master, care este eliberat de presiunea cererilor de citire. Prin direcționarea inteligentă a traficului, aplicația poate scala operațiile de citire prin adăugarea de noduri slave suplimentare, dacă este necesar, fără a afecta performanța scrierilor critice.
...
\begin{itemize}
	\item Fluxurile de Căutare și Vizualizare Proprietăți: Acestea sunt preponderent "read-heavy" (ex., căutări după destinație, vizualizarea detaliilor proprietăților, calculul disponibilității). Toate aceste interogări sunt direcționate către nodul Slave de pe VM1. Această abordare reduce drastic sarcina pe Master, asigurând timpi de răspuns foarte rapizi pentru utilizatori, chiar și sub un volum mare de cereri concurente de citire. Performanța agregată a sistemului este mult îmbunătățită.
	\item Fluxurile de Înregistrare și Autentificare: Deși implică un mix de operații (scrierea datelor de utilizator la înregistrare, citirea la autentificare), componenta de scriere (INSERT) este gestionată de Master, în timp ce citirile repetate pentru autentificare pot fi direcționate către Slave. Acest lucru previne blocajele și asigură un proces fluid de acces în sistem.
...
Implementarea și testarea unei arhitecturi distribuite, multi-nod, precum cea a sistemului de rezervări, necesită un mediu flexibil, izolat și ușor de gestionat. Infrastructura de virtualizare a jucat un rol crucial în acest proces, permițând crearea unui mediu de dezvoltare și testare care reproduce fidel condițiile de producție, fără costurile și complexitatea hardware-ului fizic. Cele două mașini virtuale (VM1 și VM2)  au fost esențiale în validarea conceptelor de scalabilitate și înaltă disponibilitate.

\subsection{Crearea unui Mediu Izolat și Consistent}
Virtualizarea permite crearea de mașini virtuale sau containere care rulează sisteme de operare și aplicații într-un mediu izolat de hardware-ul gazdă și de alte mașini virtuale. Această izolare este vitală pentru:
\begin{itemize}
	\item \textbf{Consistență:} Fiecare mediu de dezvoltare/testare poate fi configurat identic, asigurând că un comportament observat pe o mașină se va reproduce pe alta. Aceasta elimină problema clasică a "funcționează pe mașina mea".
...
\end{itemize}
Instrumente precum \texttt{Docker} (cu \texttt{Docker Compose} pentru orchestarea mai multor servicii) sunt deosebit de utile în acest sens, permițând definirea mediului (aplicație \texttt{Flask}, bază de date \texttt{MySQL}, \texttt{Nginx}) într-un fișier de configurare (\texttt{Dockerfile}, \texttt{docker-compose.yml}) care poate fi versionat și partajat.
...
\begin{itemize}
	...
	\item \textbf{Testarea scalabilității:} Prin lansarea rapidă de noi mașini virtuale sau containere, se poate simula o creștere a traficului și se poate evalua cum răspunde sistemul, confirmând că \texttt{load balancing}-ul și scalarea funcționează conform așteptărilor.
...
\end{itemize}
Această capacitate de a "rupe" sistemul în mod controlat în timpul testării este inestimabilă pentru a identifica și corecta punctele slabe înainte ca aplicația să ajungă în producție, unde astfel de defecțiuni ar avea consecințe mult mai grave.
...
Asigurarea scalabilității și a fiabilității este un obiectiv fundamental al acestei lucrări, dat fiind contextul unei aplicații web de rezervări destinată să gestioneze un volum mare de utilizatori și interacțiuni concurente. Testarea de performanță, cunoscută și sub denumirea de "load testing", este o disciplină esențială în ingineria software, ce permite evaluarea comportamentului unui sistem sub sarcini specifice. Aceasta implică simularea unui volum semnificativ de utilizatori și cereri concurente pentru a măsura capacitatea, reziliența și timpii de răspuns ai aplicației. Prin aplicarea unor scenarii de testare realiste, se pot identifica blocajele (bottlenecks), se poate evalua stabilitatea sistemului și se poate determina limita superioară a capacității sale de a gestiona traficul intens. Scopul principal al acestei etape a fost de a valida eficiența soluțiilor de scalabilitate implementate (Nginx load balancing și replicarea MySQL Master-Slave) în contextul unei aplicații web funcționale.

\section{Alegerea Instrumentului de Testare: Locust}
Pentru realizarea testelor de performanță, s-a optat pentru instrumentul open-source Locust. Alegerea Locust a fost justificată de următoarele avantaje esențiale care se aliniază cerințelor proiectului:
...
\begin{itemize}
	\item \textbf{Simularea Căutărilor (Read-Heavy):} Acest scenariu a simulat utilizatori care efectuează căutări repetate de proprietăți, vizualizează detaliile acestora și navighează prin rezultate. A fost conceput pentru a testa performanța operațiilor de citire din baza de date și eficiența distribuției cererilor de citire către nodul Slave, precum și capacitatea Nginx de a gestiona un volum mare de cereri GET.
	\item \textbf{Simularea Înregistrărilor/Autentificărilor:} Scenariul a testat procesele de înregistrare a utilizatorilor noi și de autentificare. Acest lucru a implicat un mix de operații de scriere (INSERT pentru înregistrare) și citire (SELECT pentru autentificare), punând la încercare atât nodul Master al bazei de date, cât și logica de sesiune din Flask.
	\item Simularea Procesului de Rezervare (Write-Heavy/Transactional): Acesta a fost cel mai complex scenariu, simulând un flux complet de rezervare, de la căutare la finalizarea tranzacției. A implicat multiple operații de scriere și actualizare în baza de date (INSERT, UPDATE) și a testat performanța nodului Master și capacitatea sistemului de a gestiona tranzacții ACID complexe sub sarcină.
	
	\item \textbf{Testarea Rezilienței și Failover-ului:} Pe lângă testele de sarcină, au fost simulate scenarii de eșec (ex: oprirea unei instanțe Flask, oprirea MySQL Master) pentru a valida comportamentul de failover al Nginx și al replicării MySQL, și pentru a măsura timpul de recuperare a serviciului.
	
\end{itemize}

\section{Implementarea Testelor cu Locust}
Un test \texttt{Locust} este definit printr-o clasă \texttt{User} în \texttt{Python}, care moștenește de la \texttt{HttpUser} (pentru aplicații web). Această clasă conține atribute precum \texttt{host} (adresa \texttt{Nginx}-ului), \texttt{wait\_time} (intervalul de așteptare între cereri) și metode decorate cu \texttt{@task} care definesc acțiunile pe care utilizatorii virtuali le vor executa.

\subsection{Exemplu de Comportament de Utilizator (User Behavior) pentru Căutări}
Acest fragment de cod \texttt{Python} ilustrează un scenariu simplificat pentru testarea căutărilor. \ref{lst:locust_search_task}

În acest fragment, client.get() simulează o cerere GET către server. Atributul host din clasa WebsiteUser indică adresa URL de bază a aplicației testate (de obicei Nginx load balancer).

\subsection{Exemplu de Comportament de Utilizator pentru Rezervări (Flux Complet)}
Acest fragment de cod \texttt{Python} ilustrează un scenariu mai complex, care include autentificarea și crearea unei rezervări. \ref{lst:locust_booking_task}

Acest task demonstrează cum Locust poate simula interacțiuni secvențiale, unde răspunsul unei cereri poate influența cererea următoare (ex: extragerea unui ID de proprietate din rezultatele căutării pentru a vizualiza proprietatea respectivă).
...
\begin{enumerate}
	...
	\item \textbf{Monitorizare Resurse Sistem:} Concomitent cu testele Locust, resursele hardware ale fiecărei mașini (Host, VM1, VM2) sunt monitorizate cu instrumente precum htop (pentru CPU, RAM, procese pe Linux), top (similar htop), glances (un instrument de monitorizare cross-platform) și Task Manager (pentru Windows). Instrumente precum Prometheus și Grafana pot fi, de asemenea, utilizate pentru o monitorizare mai avansată a metricilor de performanță (CPU, RAM, I/O, trafic de rețea) la nivel de sistem de operare și aplicație.
...
\begin{itemize}
	...
	\item Mașina Virtuală 1 (VM1): A fost configurată ca server MySQL Slave, dedicat operațiilor de citire și replicării datelor de la Master. Această VM a fost utilizată și pentru a rula workerii Locust, generând trafic intens către aplicație.
	\item Mașina Virtuală 2 (VM2): A găzduit o instanță secundară a aplicației Flask, care a primit cereri distribuite de la Nginx. Această mașină a jucat un rol cheie în demonstrarea scalabilității orizontale a stratului de aplicație. Această distribuție a permis simularea realistă a traficului și a interacțiunilor între componentele unui sistem distribuit, oferind o platformă robustă pentru testele de performanță.
...
Acest capitol detaliază pașii specifici pentru fiecare caz de testare, menit să valideze diferitele aspecte ale scalabilității și rezilienței arhitecturii sistemului de rezervări. Fiecare caz va fi executat utilizând Locust pentru generarea sarcinii și instrumente de monitorizare la nivel de sistem pentru colectarea metricilor.

\subsection{Cazul 1: Aplicația Flask Monolitică (Baseline - Fără Scalare/Load Balancing)}

Acest caz de testare stabilește o linie de bază a performanței, simulând o arhitectură tradițională, monolitică, pentru aplicația web de rezervări, fără soluții active de scalare sau load balancing. Obiectivul principal al acestui test este de a înregistra metricile de performanță ale sistemului în condiții inițiale, servind ca punct de referință pentru a evalua ulterior îmbunătățirile aduse de soluțiile distribuite implementate.

\subsubsection{Configurația Mediului}

În vederea reproducerii configurației monolitice cadrul de testare a fost aranjat simplist amplasând toate elementele indispensabile pe un singur sistem.

\begin{enumerate}
	\item Sistemul Gazdă (Unitatea de Procesare Principală):
	\begin{itemize}
		\item O unică copie a aplicației web Flask a fost demarată și operat pe portul implicit 5000 și s-a ocupat atât de logica funcțională cât și deservirea nemijlocită a cererilor HTTP.
		\item Serverul principal MySQL a fost operațional pe Sistemul Gazdă administrând toate tranzacțiile cu baza de date operațiuni de lectură și scriere.
	\end{itemize}
	\item Instanța Virtuală 1 (VM1): Aceasta a fost dezactivată și nu s-a folosit pe parcursul testării iar Serverul secundar MySQL nu a fost inoperant.
	\item Instanța Virtuală 2 (VM2): Aceasta a fost dezactivată și nu s-a folosit pe parcursul testării și Cealaltă copie Flask nu a fost inoperantă.
	\item Nginx: Componenta Nginx a fost dezactivată și nu a participat în gestionarea traficului iar Solicitările Locust au fost îndreptate nemijlocit către copia Flask de pe Sistemul Gazdă evitând orice dispozitiv de echilibrare a sarcinii sau proxy invers.
	\item Depozitul de Date: Copia principală MySQL de pe Sistemul Gazdă a fost legată la aplicația web Flask și Depozitul de date a fost alimentat cu o colecție de date pentru testare circa 150 utilizatori 300 proprietăți 900 unități locative produs cu programul generate\_test\_data.py iar această etapă pregătitoare a garantat un colecție uniformă de date pentru operațiunile de lectură și scriere pe durata testării.
\end{enumerate}

\subsubsection{Pregătirea și Rularea Testului Locust}
Pentru a executa testul în acest scenariu monolitic, au fost urmați pașii de configurare și lansare a Locust:
\begin{enumerate}
	\item Configurația locustfile.py: Fișierul de testare Locust a fost configurat pentru a direcționa traficul direct către instanța Flask de pe Host. Linia host din locustfile.py a fost setată la host = "http://localhost:5000". Task-urile au inclus simularea de căutări (operații de citire), vizualizarea detaliilor proprietăților și, în cazul utilizatorilor logați, interacțiuni cu pagina de profil și, la o pondere mai mică, fluxuri de înregistrare și rezervare (operații de scriere).
	\item Lansarea Aplicației Flask: Instanța aplicației Flask a fost pornită manual pe Host, asigurându-se că este accesibilă pe portul specificat (ex: python app.py).
	...
	\end{itemize}
	\item Start Test: Testul a fost demarat prin apăsarea butonului "Start swarming" și lăsat să ruleze pentru o perioadă de cel puțin 5-10 minute odată ce numărul maxim de utilizatori a fost atins.
	
\end{enumerate}

\subsubsection{Colectarea Metricilor și Observații}

Pe durata execuției testului, s-au colectat și analizat o serie de metrici de performanță pentru a înțelege comportamentul sistemului monolitic sub sarcină. Observațiile au fost înregistrate pentru a identifica eventualele blocaje.
...
\section{Analiza Rezultatelor și Identificarea Limitărilor}
Analiza detaliată a datelor colectate este crucială pentru a trage concluzii relevante despre performanța și scalabilitatea arhitecturii.
\begin{itemize}
	\item Indicatori Cheie de Performanță (KPIs): Se evaluează timpii de răspuns medii și percentilele (ex: P90, P95, P99) pentru a înțelege experiența utilizatorului. O rată mare de eșec indică probleme de stabilitate.
	
	\item Identificarea Blocajelor (Bottlenecks): Prin corelarea metricilor Locust cu monitorizarea resurselor sistemului, se pot identifica componentele care limitează performanța:
	\begin{itemize}
		\item CPU-bound: Dacă utilizarea CPU este la 100\% pe serverele Flask, înseamnă că logica aplicației este intensivă.
		
		\item Memory-bound: Dacă RAM-ul este la limită și se observă swap-in/out, memoria este un factor limitator.
		
		\item I/O-bound: Dacă discul (I/O) este supraîncărcat, baza de date sau scrierea log-urilor poate fi bottleneck-ul.
		
		\item Network-bound: Lățimea de bandă a rețelei poate limita traficul.

	\end{itemize}
	
	\item Comportamentul Load Balancer-ului: Se analizează log-urile Nginx pentru a verifica distribuția cererilor între serverele Flask. O distribuție inegală sau o rată mare de erori pentru un anumit server pot indica probleme.
	
	\item Comportamentul Replicării MySQL: Se monitorizează lag-ul de replicare (timpul dintre scrierea pe Master și aplicarea pe Slave) pentru a asigura că Slave-ul este suficient de actualizat. Se analizează performanța nodului Master sub operații de scriere și a nodului Slave sub operații de citire.
	\item Concluzii din Teste: Se trage o concluzie generală despre eficiența soluțiilor de scalabilitate implementate, evidențiind succesul în gestionarea traficului și identificând eventualele limitări care ar putea necesita optimizări suplimentare în direcțiile viitoare.
\end{itemize}

\section{Cazul 1: Aplicația Flas\textbf{Aplicația Flask Monolitică} (Baseline - Fără Scalare/Load Balancing)}

Acest caz de testare stabilește o linie de bază a performanței, simulând o arhitectură tradițională, monolitică, pentru aplicația web de rezervări, fără soluții active de scalare sau load balancing. Obiectivul principal al acestui test este de a înregistra metricile de performanță ale sistemului în condiții inițiale, servind ca punct de referință pentru a evalua ulterior îmbunătățirile aduse de soluțiile distribuite implementate. Rezultatele obținute în acest scenariu vor evidenția limitările inerente ale unei arhitecturi neoptimizate pentru trafic intens, justificând astfel necesitatea abordărilor complexe de scalare.
...
\section{Concluzii}
Lucrarea de licență a abordat studiul și implementarea unei aplicații web complexe de tip sistem de rezervări de proprietăți, concentrându-se pe aspectele esențiale legate de arhitectură, implementare și scalabilitate. Prin disecția detaliată a fluxurilor funcționale principale și a mecanismelor de suport, s-a demonstrat o înțelegere aprofundată a provocărilor și soluțiilor aferente dezvoltării de aplicații distribuite și s-a reușit construirea unui sistem funcțional și robust.

Obiectivele principale ale lucrării au fost atinse cu succes:
\begin{itemize}
	\item A fost proiectată și implementată o arhitectură \texttt{N-Tier} modulară, separând clar straturile de prezentare (\texttt{frontend}), logică de afaceri (\texttt{backend}) și date. Această abordare a asigurat o bună mentenanță, testabilitate și posibilități de scalare independentă a componentelor.
	\item Au fost implementate fluxurile esențiale ale aplicației: înregistrarea și autentificarea utilizatorilor, căutarea și vizualizarea proprietăților, precum și procesul complex de creare a rezervărilor. S-a pus un accent deosebit pe securitatea datelor sensibile, prin utilizarea unor mecanisme criptografice robuste precum \texttt{bcrypt} pentru hashing-ul parolelor.
	\item A fost dezvoltată o logică avansată pentru prevenirea \texttt{double booking}-ului, folosind interogări SQL de suprapunere și gestionarea tranzacțională \texttt{ACID} pentru a asigura integritatea și consistența datelor în timpul operațiilor critice de rezervare.
	\item S-au implementat fluxuri de management pentru proprietari (adăugare/editare proprietăți, vizualizare rezervări primite) și un panou de administrare, cu un control granular al accesului bazat pe roluri (\texttt{session['is\_admin']}), esențial pentru supravegherea și controlul sistemului.
	\item S-au analizat și integrat componente cheie de infrastructură, precum \texttt{Nginx} pentru \texttt{load balancing} și servirea fișierelor statice, și replicarea \texttt{MySQL Master-Slave} pentru înaltă disponibilitate și scalarea operațiilor de citire. A fost subliniat rolul infrastructurii de virtualizare în dezvoltarea și testarea acestor componente într-un mediu controlat.
\end{itemize}

Punctele forte ale arhitecturii și implementării includ:
\begin{itemize}
	\item **Securitate robustă:** Implementarea \texttt{bcrypt} și a sesiunilor criptografice asigură un nivel înalt de protecție a credențialelor utilizatorilor și a integrității sesiunilor.
	\item **Integritate tranzacțională:** Utilizarea tranzacțiilor \texttt{ACID} garantează că operațiile critice, cum ar fi rezervările, sunt sigure și consistente, prevenind stările de date inconsistente.
	\item **Scalabilitate și reziliență:** Prin utilizarea \texttt{Nginx} pentru \texttt{load balancing} și a replicării \texttt{MySQL}, aplicația este pregătită să gestioneze volume mari de trafic și să funcționeze continuu chiar și în cazul unor defecțiuni ale serverelor.
	\item **Modularitate:** Structura \texttt{N-Tier} și separarea clară a responsabilităților contribuie la un cod ușor de înțeles, de extins și de depanat.
	\item **Experiența utilizatorului:** Designul intuitiv al interfețelor și prezența unor mecanisme de căutare "fuzzy" și de recomandare a camerelor contribuie la o experiență utilizator pozitivă.
\end{itemize}

Desigur, pe parcursul dezvoltării, au existat și lecții învățate, cum ar fi complexitatea gestionării consistente a datelor în medii distribuite și importanța validării stricte a intrărilor de la utilizatori la nivel de backend. A fost, de asemenea, subliniată valoarea testării exhaustive a scenariilor de eșec pentru a asigura robustetea sistemului.

\section{Direcții Viitoare de Dezvoltare}
Proiectul actual servește ca o fundație solidă pentru o platformă de rezervări de proprietăți, dar există numeroase oportunități de extindere și îmbunătățire pentru a-l transforma într-o soluție completă și competitivă pe piață. Direcțiile viitoare de dezvoltare pot fi structurate pe mai multe paliere:

\subsection{Extensii ale Funcționalităților Aplicației}
\begin{itemize}
	\item **Sistem de Mesagerie Integrat:** Implementarea unui sistem de mesagerie directă între chiriași și proprietari ar facilita comunicarea și ar îmbunătăți experiența utilizatorului. Aceasta ar putea include notificări în timp real.
	\item **Sistem de Recenzii și Rating-uri:** Permiterea utilizatorilor să lase recenzii și rating-uri pentru proprietăți ar construi încredere în comunitate și ar oferi informații valoroase pentru alți potențiali chiriași. Acest lucru ar implica un nou tabel în baza de date (\texttt{reviews}) și logică de agregare a rating-urilor.
	\item **Integrare cu Sisteme de Plăți Online:** Implementarea unui gateway de plată (ex: Stripe, PayPal) ar permite utilizatorilor să finalizeze rezervările direct prin platformă, automatizând procesul de plată și managementul financiar pentru proprietari. Aceasta ar necesita gestionarea securizată a tranzacțiilor financiare și integrarea cu \texttt{API}-uri externe.
	\item **Funcționalități Avansate de Căutare și Filtrare:** Adăugarea de filtre suplimentare (ex: facilități, tip de proprietate, interval de preț) și o hartă interactivă cu proprietăți ar îmbunătăți semnificativ experiența de căutare.
	\item **Notificări Automate:** Implementarea notificărilor prin email sau \texttt{SMS} pentru confirmări de rezervare, anulări, memento-uri sau mesaje noi ar spori angajamentul utilizatorilor.
	\item **Gestionarea Disponibilității la Nivel de Calendar:** Un calendar interactiv avansat pentru proprietari care să le permită să blocheze/deblocheze date specifice rapid, și pentru chiriași să vizualizeze disponibilitatea clar.
\end{itemize}

\subsection{Îmbunătățiri ale Performanței și Scalabilității Infrastructurii}
\begin{itemize}
	\item **Introducerea unui Strat de Caching:** Utilizarea unui sistem de \texttt{caching} (ex: \texttt{Redis}, \texttt{Memcached}) pentru datele accesate frecvent (ex: rezultate de căutare populare, detalii proprietăți) ar reduce numărul de cereri către baza de date și ar îmbunătăți semnificativ timpii de răspuns.
	\item **Arhitectura de Microservicii:** Pe termen lung, aplicația ar putea fi refactorizată într-o arhitectură de microservicii, unde fiecare funcționalitate majoră (autentificare, proprietăți, rezervări, plăți) este un serviciu independent. Acest lucru ar permite scalarea și dezvoltarea independentă a fiecărui modul.
	\item **Utilizarea Serviciilor Cloud Gestiionate:** Migrarea bazei de date (\texttt{MySQL}) către un serviciu de bază de date gestionat în cloud (ex: \texttt{Amazon RDS}, \texttt{Google Cloud SQL}) ar simplifica managementul infrastructurii, ar oferi \texttt{scalability out-of-the-box} și \texttt{failover} automat. Similar, utilizarea \texttt{load balancer}-ilor cloud (ex: \texttt{AWS ALB}) ar eficientiza distribuția traficului.
	\item **Content Delivery Network (CDN):** Implementarea unui \texttt{CDN} pentru servirea fișierelor statice (imagini, CSS, JS) ar reduce latența pentru utilizatorii aflați la distanță geografică de serverele principale, îmbunătățind viteza de încărcare a paginilor.
\end{itemize}

\subsection{Procese de Dezvoltare și Deploy}
\begin{itemize}
	\item **Integrare Continuă/Livrare Continuă (\texttt{CI/CD}):** Implementarea unui pipeline \texttt{CI/CD} (folosind instrumente precum \texttt{GitLab CI}, \texttt{Jenkins}, \texttt{GitHub Actions}) ar automatiza procesele de testare, construire și \texttt{deploy} al aplicației. Acest lucru ar asigura că fiecare modificare a codului este testată riguros și poate fi implementată rapid și fiabil în producție.
	\item **Monitorizare Avansată și Logging:** Adăugarea unor instrumente de monitorizare detaliată (ex: \texttt{Prometheus}, \texttt{Grafana}) și a unei soluții centralizate de \texttt{logging} (ex: \texttt{ELK Stack} - \texttt{Elasticsearch}, \texttt{Logstash}, \texttt{Kibana}) ar permite echipei de operare să identifice și să rezolve rapid problemele, să analizeze performanța și să înțeleagă mai bine comportamentul utilizatorilor.
	\item **Testare Automată:** Extinderea acoperirii testelor automate (unitare, de integrare, funcționale) pentru a asigura calitatea și stabilitatea aplicației pe măsură ce se adaugă noi funcționalități.
\end{itemize}



\section{Încheiere}
Lucrarea de licență a demonstrat capabilitatea de a proiecta, implementa și testa o aplicație web complexă, utilizând tehnologii moderne și principii de arhitectură software. Soluția dezvoltată servește ca o platformă funcțională, sigură și scalabilă, care poate fi extinsă în multiple direcții pentru a răspunde cerințelor unui sistem de rezervări la scară reală. Experiența acumulată în procesul de dezvoltare subliniază importanța colaborării strânse dintre conceptele teoretice și aplicarea lor practică în construirea de sisteme software robuste și eficiente.
