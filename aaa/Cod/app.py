...
# Server standard din Python, nu necesită instalare
from wsgiref.simple_server import make_server
# Adaugă aceste linii la început, lângă celelalte importuri
import cProfile
import pstats
...
# --- CONFIGURARE NOUĂ PENTRU MASTER/SLAVE READ/WRITE SPLIT ---

MASTER_DB_CONFIG = {
    'host': '192.168.50.1',  # IP-ul PC-ului HOST unde este Master DB
    'user': 'app_user',
    'password': 'parola_aplicatiei',  # Asigură-te că aceasta e parola corectă
    'database': 'rezervari',
    'connection_timeout': 10
}
SLAVE_DB_CONFIG = {
    'host': '192.168.50.2',  # IP-ul mașinii virtuale VM1 (Slave DB)
    'user': 'app_user',
    'password': 'parola_aplicatiei',  # Asigură-te că aceasta e parola corectă
    'database': 'rezervari',
    'connection_timeout': 10
...
# Pool-urile sunt None la început. Vor fi create la prima cerere de către fiecare proces.
master_pool = None
slave_pool = None
...
def get_master_pool():
    ...
    if master_pool is None:
        print(f"Proces [{os.getpid()}] creează pool pentru MASTER DB...")
        master_pool = pooling.MySQLConnectionPool(pool_name=f"master_{os.getpid()}", pool_size=32, **MASTER_DB_CONFIG)
    return master_pool

def get_slave_pool():
    global slave_pool
    if slave_pool is None:
        print(f"Proces [{os.getpid()}] creează pool pentru SLAVE DB...")
        slave_pool = pooling.MySQLConnectionPool(pool_name=f"slave_{os.getpid()}", pool_size=32, **SLAVE_DB_CONFIG)
    return slave_pool

@contextmanager
def get_master_connection():
    """Obține o conexiune de SCRIERE de la pool-ul Master."""
    cnx = None
    try:
        cnx = get_master_pool().get_connection()
        yield cnx
    finally:
        if cnx: cnx.close()
...
def get_slave_connection():
    """Obține o conexiune de CITIRE de la pool-ul Slave."""
    cnx = None
    try:
        cnx = get_slave_pool().get_connection()
        yield cnx
    finally:
        if cnx: cnx.close()
...
def home():
    try:
        # CITIRE -> SLAVE
        with get_slave_connection() as cnx:
...
def register():
    if request.method == 'POST':
        ...
        try:
            # SCRIERE -> MASTER
            with get_master_connection() as cnx:
                with get_db_cursor(cnx) as cursor:
...
def login():
    print("Login route hit!") # Add this
   
    if request.method == 'POST':
        username = request.form['username']
        password = request.form['password']
        try:
            # CITIRE critică (parolă) și SCRIERE sesiune -> MASTER
            with get_master_connection() as cnx:
                with get_db_cursor(cnx) as cursor:
    ...
    try:
        ...
        with get_master_connection() as cnx:
            with get_db_cursor(cnx) as cursor:
                ...
                # CITIRE (pentru afișare formular) -> tot MASTER pentru consistență imediată
                cursor.execute("SELECT * FROM users WHERE user_id = %s", (user_id,))
...
# Nu uita să adaugi 'import time' la începutul fișierului app.py
...
def search_results():
    try:
        ...
        # --- MĂSURĂM TIMPUL DE CONECTARE ---
        t_start_connect = time.time()
        with get_slave_connection() as cnx:
            t_end_connect = time.time()
            connect_time = (t_end_connect - t_start_connect) * 1000 # Convertim în milisecunde
            if connect_time > 100: # Afișăm doar dacă durează mai mult de 100ms
                print(f"!!! ATENTIE: Timpul de obtinere a conexiunii a fost: {connect_time:.2f} ms")
            # ------------------------------------
            ...
            with get_db_cursor(cnx) as cursor:
                ...
                # --- MĂSURĂM TIMPUL PRIMEI INTEROGĂRI ---
                t_start_q1 = time.time()
                cursor.execute(base_query, tuple(params))
                candidate_properties = cursor.fetchall()
                t_end_q1 = time.time()
                q1_time = (t_end_q1 - t_start_q1) * 1000
                if q1_time > 100:
                    print(f"--- Query 1 (cautare proprietati) a durat: {q1_time:.2f} ms")
                # ----------------------------------------
                ...
                for prop in candidate_properties:
                    ...
                    # --- MĂSURĂM TIMPUL INTEROGĂRII DIN BUCLĂ ---
                    t_start_q2 = time.time()
                    cursor.execute(room_query, (prop['property_id'], start_date, end_date))
                    available_room_groups = cursor.fetchall()
                    t_end_q2 = time.time()
                    q2_time = (t_end_q2 - t_start_q2) * 1000
                    if q2_time > 100:
                         print(f"--- Query 2 (cautare camere pentru prop {prop['property_id']}) a durat: {q2_time:.2f} ms")
                    # ---------------------------------------------
    ...
    try:
        ...
        with get_master_connection() as cnx:
            with get_db_cursor(cnx) as cursor:
                ...
                # CITIRE (formular) -> tot MASTER pentru consistență
...
def check_room_type_reservations(property_id, room_type_name_encoded):
    ...
    if 'user_id' not in session: return jsonify({"has_reservations": False}), 401
    try:
        # CITIRE -> SLAVE
        with get_slave_connection() as cnx:
            with cnx.cursor(dictionary=True) as cursor:
