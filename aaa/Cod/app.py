        ...
        try:
            with get_db_connection() as cnx:
                with get_db_cursor(cnx, dictionary=False) as cursor:
                    # Verifică dacă utilizatorul sau emailul există deja
                    cursor.execute("SELECT * FROM users WHERE username = %s OR email = %s", (username, email))
                    if cursor.fetchone():
                        flash("Numele de utilizator sau emailul există deja.", "error")
            ...
            flash(f"A apărut o eroare la înregistrare: {e}", "error")
            return redirect(url_for('register'))
    ...
    try:
        with get_db_connection() as cnx:
            with get_db_cursor(cnx) as cursor:
                ...
                return render_template('profile.html', username=session.get('username'), rezervari=rezervari, proprietati=proprietati, logged_in=True)
    except Exception as e:
...
def edit_profile():
    ...
    try:
        with get_db_connection() as cnx:
            with get_db_cursor(cnx) as cursor:
                if request.method == 'POST':
                    ...
                    # Verifică dacă noul username sau email sunt deja folosite de altcineva
                    cursor.execute("SELECT user_id FROM users WHERE (username = %s OR email = %s) AND user_id != %s", (new_username, new_email, user_id))
                    if cursor.fetchone():
                        flash("Numele de utilizator sau emailul este deja folosit de alt cont.", "error")
        ...
        print(f"Eroare la edit_profile: {e}")
        flash("A apărut o eroare la editarea profilului.", "error")
        return redirect(url_for('profile'))
