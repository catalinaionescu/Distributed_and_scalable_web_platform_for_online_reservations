...
import socket # <--- ADAUGĂ ACEST IMPORT LA ÎNCEPUTUL FIȘIERULUI

app = Flask(__name__)
app.secret_key = "cheie_super_secreta_perta_proiect_v6"

# --- CONFIGURARE BAZĂ DE DATE (rămâne neschimbată) ---
# ... (codul tău de configurare a bazei de date este aici) ...
MASTER_DB_CONFIG = {
    'host': '192.168.50.1',
    'user': 'app_user',
...
# --- START BLOC NOU: Funcție de verificare status server ---
def check_server_status(host, port):
    """Verifică dacă un server este online la un host și port specificat."""
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.settimeout(2)  # Așteaptă maxim 2 secunde pentru conexiune
    try:
...
# --- Restul funcțiilor tale (is_logged_in, format_ro_date, etc.) rămân aici ---
# ...
def is_logged_in():
...
def admin():
    ...
    # --- START MODIFICARE ---
    # Verificăm statusul serverului IIS la adresa IP corectă
    ...
    try:
        ...
        with get_slave_connection() as cnx:
            with get_db_cursor(cnx) as cursor:
                ...
                # --- START MODIFICARE ---
                # Trimitem statusul real către template
...
def check_room_type_reservations(property_id, room_type_name_encoded):
    ...
    try:
        # Folosim SLAVE pentru citiri
        with get_slave_connection() as cnx:
            with cnx.cursor(dictionary=True) as cursor:
                # Verificăm dacă utilizatorul curent este proprietarul
                cursor.execute("SELECT owner_id FROM properties WHERE property_id = %s", (property_id,))
                ...
                # Verificăm dacă există rezervări active sau viitoare ('confirmed')
                cursor.execute("""
                    SELECT 1 FROM reservations res 
                    JOIN rooms r ON res.room_id = r.idrooms 
                    WHERE r.property_id = %s AND r.name = %s 
                ...
                if cursor.fetchone():
                    # Dacă găsește cel puțin o rezervare activă, blochează ștergerea
                    return jsonify({
                        "can_delete_type_now": False,
                        "message": "Acest tip de cameră nu poate fi șters deoarece are rezervări active sau viitoare."
                    })
                
                # Dacă nu există rezervări active/viitoare, permite ștergerea
                return jsonify({
