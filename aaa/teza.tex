...
\section{Introducere: Filozofia Arhitecturală și Stiva Tehnologică}
...
\begin{enumerate}
    \item \textbf{Performanță:} Analiza și aplicarea tehnicilor de optimizare a timpului de răspuns, atât pentru operațiuni simple de afișare, cât și pentru procesări complexe precum căutarea și rezervarea.
    \item \textbf{Modularitate:} Proiectarea sistemului conform unei paradigme modulare, în care fiecare componentă deține o responsabilitate clar definită, facilitând testarea, mentenanța și actualizarea independentă.
    \item \textbf{Scalabilitate:} Implementarea unei arhitecturi capabile să susțină o creștere a numărului de utilizatori fără o degradare semnificativă a performanței, prin strategii ce permit scalabilitatea orizontală.
    \item \textbf{Testabilitate:} Construirea componentelor software într-o manieră care să permită validarea funcțională prin teste unitare și de integrare, asigurând un nivel înalt de fiabilitate.
\end{enumerate}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\columnwidth]{figuri/mod.png}
...
\begin{itemize}
    ...
    \item \textbf{mysql.connector:} Biblioteca oficială pentru conectarea la MySQL. S-a utilizat sub-modulul \textit{pooling} pentru a implementa \textit{Connection Pooling}, o tehnică de optimizare ce menține un set de conexiuni deschise și refolosibile.
...
\section{Fluxul 1 (End-to-End): Înregistrare și Autentificare Utilizator}


Primul și cel mai fundamental flux de interacțiune cu platforma este cel prin care un vizitator devine un utilizator înregistrat și autentificat. Acest parcurs, cunoscut în terminologia de specialitate ca \textit{user onboarding}, este esențial nu doar pentru personalizarea experienței, ci și pentru stabilirea unui perimetru de securitate în jurul funcționalităților tranzacționale, precum efectuarea de rezervări. Analiza acestui flux end-to-end, prezentată în continuare, va acoperi interfața vizuală, logica de procesare din backend și mecanismele de securitate implementate.

\subsection{Interfața Frontend și Principii de Design (UI/UX)}

Poarta de intrare în ecosistemul aplicației este reprezentată de formularele de înregistrare și autentificare. Proiectarea acestora nu este un aspect trivial, ci o decizie conștientă care influențează direct rata de conversie a vizitatorilor în utilizatori activi. Abordarea adoptată a fost una minimalistă, focusată pe eliminarea oricărei fricțiuni și pe ghidarea intuitivă a utilizatorului către finalizarea acțiunii.

\subsubsection{Formularul de Înregistrare: Primul Contact cu Platforma}

Punctul de intrare pentru un nou membru este materializat de pagina de înregistrare, servită de șablonul \texttt{register.html}. Această componentă de interfață a fost proiectată sub principiul **colectării progresive a datelor** (\textit{progressive disclosure}). În loc să copleșim utilizatorul cu un formular extins, am solicitat doar setul minim de atribute indispensabile pentru crearea unui cont funcțional, așa cum se poate observa în Figura \ref{fig:form_register}.

Fiecare câmp a fost selectat cu un scop precis:
\begin{itemize}
    \item \textbf{Numele de utilizator:} Servește ca identitate publică în cadrul platformei, de exemplu, în secțiunea de recenzii.
    \item \textbf{Adresa de e-mail:} Acționează ca identificator unic, primar, în sistem și principalul canal de comunicare pentru notificări tranzacționale (confirmări de rezervare, etc.).
    \item \textbf{Parola:} Reprezintă cheia de acces, al cărei tratament criptografic în backend este un pilon central al securității aplicației.
\end{itemize}

Validarea la nivel de client, implementată prin atribute HTML5 native precum \texttt{required} și \texttt{type="email"}, constituie prima barieră de integritate a datelor. Această tehnică oferă utilizatorului un feedback imediat, în timp real, asupra corectitudinii informațiilor introduse, reducând numărul de cereri invalide care ajung la server și, implicit, optimizând utilizarea resurselor și fluidizând experiența de înregistrare.

\begin{figure}[H]
    \centering
    % TODO: Înlocuiește 'placeholder.png' cu o captură de ecran a paginii tale de înregistrare
    \includegraphics[width=1\columnwidth]{figuri/newuser.png} 
    \caption{Interfața formularului de înregistrare, proiectată pentru simplitate și claritate.}
    \label{fig:form_register}
\end{figure}

\subsubsection{Formularul de Autentificare: Calea de Acces Rapid}

Odată ciclul de înregistrare finalizat, parcursul de revenire al utilizatorului este deservit de pagina de autentificare (\texttt{login.html}). Design-ul acesteia, prezentat în Figura \ref{fig:form_login}, este deliberat și mai minimalist, respectând principiul UX al **eliminării distragerilor**. Pagina are un singur scop, o singură "chemare la acțiune" (\textit{Call to Action}): autentificarea.

Prin solicitarea exclusivă a celor două credențiale – e-mail și parolă – se reduce sarcina cognitivă a utilizatorului și se accelerează la maximum procesul de accesare a contului. Acest formular acționează ca o poartă securizată; la submitere, acesta inițiază un dialog cu backend-ul, unde se va efectua validarea criptografică a parolei, un mecanism ce va fi disecat în secțiunea următoare.

\begin{figure}[H]
    \centering
    % TODO: Înlocuiește 'placeholder.png' cu o captură de ecran a paginii tale de login
    \includegraphics[width=1\columnwidth]{figuri/login.png} 
    \caption{Interfața formularului de autentificare, focusată pe funcționalitate.}
    \label{fig:form_login}
\end{figure}

\subsection{Procesarea Backend și Mecanismele de Securitate}
\label{ssec:backend_auth}

În momentul în care un utilizator trimite un formular de înregistrare sau de autentificare, datele părăsesc browser-ul și ajung la serverul nostru. Aici, aplicația Flask preia controlul și inițiază un proces în mai mulți pași pentru a valida, securiza și persista aceste informații. Vom analiza fiecare mecanism tehnic implicat în acest flux.

\subsubsection{Mecanismul 1: Validarea Datelor și Prevenirea Duplicatelor}

Primul pas pe care serverul îl execută este validarea la nivel de server. Chiar dacă există validări în frontend, acestea pot fi ocolite. Prin urmare, validarea la nivel de server este esențială. Pentru ruta \texttt{/register}, aceasta înseamnă verificarea faptului că adresa de e-mail furnizată nu există deja în sistem.

Acest lucru se realizează printr-o interogare \texttt{SELECT} asupra tabelei \texttt{users}. Dacă interogarea returnează un rezultat, înseamnă că e-mailul este deja folosit, iar procesul de înregistrare este oprit. Utilizatorului i se afișează un mesaj de eroare prin mecanismul \texttt{flash} din Flask, fără a se crea un cont duplicat. Acest pas garantează integritatea și unicitatea datelor din baza de date.

\begin{lstlisting}[language=Python, 
  caption={Interogare SQL pentru prevenirea conturilor duplicate.}, 
  label={fig:duplicate_check_code}]
# Fragment de cod din app.py: Verificarea existentei contului
cursor = db_connection_pool.get_connection().cursor()
cursor.execute("SELECT * FROM users WHERE email = %s", (email,))
account = cursor.fetchone()

if account:
    flash('Contul exista deja!', 'danger')
    return redirect(url_for('register'))
# ... continua cu crearea contului daca nu exista
\end{lstlisting}



\subsubsection{Mecanismul 2: Securizarea Criptografică a Parolelor (bcrypt)}

Cel mai critic aspect de securitate în gestiunea utilizatorilor este protejarea parolelor. Stocarea parolelor în clar este o practică inacceptabilă. Platforma noastră utilizează biblioteca \textbf{bcrypt} pentru a transforma parolele într-un format securizat, non-reversibil (hash).

\textbf{La înregistrare,} procesul este următorul:
\begin{enumerate}
    \item Se ia parola introdusă de utilizator.
    \item Se generează o "sare" (\textit{salt}) – un șir de caractere aleatoriu, unic pentru fiecare utilizator.
    \item Parola și sarea sunt combinate și trecute printr-un algoritm de hashing lent (Blowfish).
\end{enumerate}

Rezultatul (`hashed\_password`) este cel care se stochează în baza de date. Deoarece sarea este unică, chiar și aceeași parolă va genera hash-uri diferite pentru utilizatori diferiți, anihilând eficiența atacurilor de tip \textit{rainbow table}.

\textbf{La autentificare,} procesul este invers, dar la fel de sigur:
\begin{enumerate}
    \item Se preia hash-ul stocat în baza de date pentru utilizatorul respectiv.
    \item Se utilizează funcția \texttt{bcrypt.checkpw()}, care extrage automat sarea din hash-ul stocat.
    \item Aplică același proces de hashing parolei introduse acum de utilizator, folosind sarea extrasă.
    \item Compară cele două hash-uri. Doar dacă sunt identice, parola este corectă.
\end{enumerate}
Astfel, parola în clar nu este niciodată manipulată sau comparată direct după înregistrarea inițială.

\begin{lstlisting}[language=Python, 
  caption={Utilizarea bcrypt pentru hasharea și verificarea parolelor.}, 
  label={fig:bcrypt_usage}]
# La inregistrare:
parola_clar = request.form['password'].encode('utf-8')
parola_hashed = bcrypt.hashpw(parola_clar, bcrypt.gensalt())
cursor.execute("INSERT INTO users (password_hash) VALUES (%s)", (parola_hashed,))

# La autentificare:
parola_formular = request.form['password'].encode('utf-8')
parola_hashed_din_bd = user['password_hash'].encode('utf-8')
if bcrypt.checkpw(parola_formular, parola_hashed_din_bd):
# Parolele se potrivesc - autentificare reusita
\end{lstlisting}




\subsubsection{Mecanismul 3: Gestiunea Stării prin Sesiuni Securizate}

Protocolul HTTP este, prin natura sa, "fără stare" (\textit{stateless}). Serverul nu își amintește de la o cerere la alta cine este utilizatorul. Pentru a rezolva această problemă, folosim sistemul de sesiuni al Flask.

După o autentificare reușită, creăm o "sesiune" pentru utilizator. Acest lucru înseamnă că stocăm în obiectul special \texttt{session} din Flask informații de identificare, de obicei ID-ul unic al utilizatorului: \texttt{session['user\_id'] = user['id']}.

Flask împachetează aceste date, le \textbf{semnează criptografic} folosind cheia secretă a aplicației (\texttt{app.secret\_key}) și le trimite browser-ului sub forma unui cookie. La fiecare cerere viitoare, browser-ul va trimite acest cookie înapoi. Serverul Flask verifică semnătura. Dacă semnătura este validă, serverul are încredere în datele din cookie și știe cine este utilizatorul. Dacă cineva încearcă să modifice cookie-ul (de exemplu, să schimbe `user\_id`), semnătura nu se va mai potrivi, iar sesiunea va fi invalidată automat. Acesta este un mecanism robust care asigură persistența stării de autentificare într-un mod sigur.

\begin{verbatim}
# La autentificare reușită, se populează sesiunea
session['loggedin'] = True
session['user_id'] = user['id']
session['username'] = user['username']

# Într-o altă rută, se accesează datele din sesiune
if 'loggedin' in session:
    current_user_id = session['user_id']
    # ... folosește ID-ul pentru a face operațiuni personalizate
\end{verbatim}
\captionof{figure}{Crearea și utilizarea unei sesiuni de utilizator în Flask.}
\label{fig:session_usage}




\subsection{Procesarea Backend și Mecanismele de Afișare}

În spatele interfeței vizuale stă un mecanism complex în backend, responsabil pentru interpretarea criteriilor utilizatorului, interogarea bazei de date și construirea dinamică a listei de rezultate relevante.

\subsubsection{Mecanismul de Căutare și Filtrare Avansată}

La trimiterea formularului de căutare, browser-ul execută o cerere \texttt{HTTP POST} către ruta \texttt{/search\_results}. Spre deosebire de o căutare simplă, logica implementată în backend execută un proces de filtrare în doi pași, mult mai precis și mai eficient:

\begin{enumerate}
    \item \textbf{Filtrarea Inițială a Proprietăților:} Într-o primă fază, se execută o interogare generală pe tabela \texttt{properties} pentru a selecta un set de "proprietăți candidate". Această interogare filtrează proprietățile pe baza locației introduse de utilizator și, important, exclude proprietățile care aparțin utilizatorului autentificat în acel moment (pentru a nu-și putea rezerva propria proprietate).

    \item \textbf{Validarea Detaliată a Disponibilității Camerelor:} În a doua fază, aplicația iterează prin fiecare proprietate candidată și execută o a doua interogare, mult mai complexă, pe tabela \texttt{rooms}. Această interogare este nucleul tehnic al motorului de căutare: ea selectează doar camerele care sunt disponibile în intervalul de date specificat de utilizator.
\end{enumerate}

Mecanismul de validare a disponibilității este implementat printr-o sub-interogare \texttt{NOT IN}. Se selectează toate camerele unei proprietăți (`idrooms`) care \textbf{nu se află} în lista de camere deja rezervate (`reservations.room\_id`) în intervalul de timp care se suprapune cu cel dorit. Logica de suprapunere a intervalelor, \texttt{NOT (end\_date <= start\_date\_cautare OR start\_date >= end\_date\_cautare)}, este esențială pentru a garanta corectitudinea rezultatelor.

Fragmentul de cod din Figura \ref{fig:search_backend_code} ilustrează această logică complexă de interogare.


 Pas 1: Filtrarea proprietăților candidate
\begin{figure}[H]
    \centering
    % TODO: Înlocuiește 'placeholder.png' cu o captură de ecran a paginii tale de login
    \includegraphics[width=1\columnwidth]{figuri/s1.png} 
    \caption{Interfața formularului de autentificare, focusată pe funcționalitate.}
    \label{s1}
\end{figure}
\begin{figure}[H]
    \centering
    % TODO: Înlocuiește 'placeholder.png' cu o captură de ecran a paginii tale de login
    \includegraphics[width=1\columnwidth]{figuri/s2.png} 
    \caption{Interfața formularului de autentificare, focusată pe funcționalitate.}
    \label{s1}
\end{figure}

\begin{lstlisting}[language=Python, caption={Logica de filtrare în doi pași și interogarea pentru validarea disponibilității camerelor.}, label={fig:search_backend_code}]
properties_that_match = []
for prop in candidate_properties:
    # Pas 2: Verificarea disp camerelor pentru fiecare proprietate
    room_query = """
        SELECT name, capacity, price, COUNT(*) as count, 
               GROUP_CONCAT(idrooms) as room_ids 
        FROM rooms 
        WHERE property_id = %s AND available = TRUE AND idrooms NOT IN (
            SELECT room_id FROM reservations 
            WHERE status = 'confirmed' AND NOT (end_date <= %s OR start_date >= %s)
        ) 
        GROUP BY name, capacity, price
    """
    cursor.execute(room_query, (prop['property_id'], start_date, end_date))
    available_room_groups = cursor.fetchall()

    # Pas 3 (Optional): Invocarea motorului de recomandzri
...
return render_template("search_results.html", properties=properties_that_match, ...)
\end{lstlisting}


\subsubsection{Motorul de Recomandări și Randarea Rezultatelor}

După identificarea grupurilor de camere disponibile, logica nu se oprește la o simplă afișare. Se invocă o funcție specializată, \texttt{get\_recommendations()}, care are rolul de a găsi combinația optimă de camere pentru a satisface cerințele utilizatorului (număr de adulți și număr de camere dorite). Această abordare transformă o simplă căutare într-un sistem inteligent de asistență în planificarea călătoriei.

Doar proprietățile pentru care se găsește o recomandare validă sunt trimise către șablonul \texttt{search\_results.html}, unde sunt randate dinamic folosind motorul Jinja2, așa cum a fost descris anterior. Rezultatul final este o listă de opțiuni nu doar disponibile, ci și perfect potrivite pentru nevoile utilizatorului, așa cum se poate vedea în Figura \ref{fig:search_results_page}.

\begin{figure}[H]
    \centering
    % TODO: Fă o captură de ecran cu pagina ta de rezultate (search_results.html)
    % după ce ai efectuat o căutare. Aceasta este dovada că totul funcționează.
    \includegraphics[width=\columnwidth]{figuri/s1.png} 
    \caption{Exemplu de pagină cu rezultatele căutării, afișând proprietățile disponibile.}
    \label{fig:search_results_page}
\end{figure}



\textbf{Gestiunea Rutelor și a Ciclului Cerere-Răspuns HTTP}

Flask gestionează cererile HTTP printr-un sistem de rutare bazat pe decoratori. Decoratorul @app.route() leagă un URL specific de o funcție Python care va fi executată atunci când acel URL este accesat.
