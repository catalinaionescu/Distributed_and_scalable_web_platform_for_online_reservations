...
Instanța aplicației configurează \texttt{app.secret\_key}, utilizată de Flask pentru semnarea criptografică a cookie-urilor de sesiune, prevenind manipularea acestora și securizând sistemul de autentificare.
.
...
Așa cum a fost prezentat în Secțiunea 2.5, Nginx are un rol esențial în menținerea scalabilității și rezilienței aplicației web, prin funcționalitățile sale de load balancing și reverse proxy. În arhitectura implementată pentru sistemul de rezervări, Nginx este configurat să gestioneze eficient traficul către instanțele aplicației Flask, contribuind direct la optimizarea performanței și disponibilității platformei.
...
Asigurarea scalabilității și fiabilității reprezintă un obiectiv central al acestei lucrări, având în vedere natura aplicației web de rezervări, care trebuie să gestioneze un număr ridicat de utilizatori și interacțiuni concurente. Testarea de performanță, sau load testing, este o disciplină esențială în ingineria software, permițând evaluarea comportamentului sistemului sub sarcini specifice. Aceasta presupune simularea unui volum semnificativ de utilizatori și cereri concurente pentru a măsura capacitatea, reziliența și timpii de răspuns ai aplicației. Prin aplicarea unor scenarii realiste, se pot identifica blocajele, se poate evalua stabilitatea sistemului și se poate determina limita maximă a traficului gestionabil. Scopul acestei etape a fost validarea eficienței soluțiilor de scalabilitate implementate, respectiv Nginx load balancing și replicarea MySQL Master-Slave, în cadrul unei aplicații web funcționale.
...
\textbf{Descriere:} Simulează comportamentul unui utilizator care explorează platforma, incluzând accesarea paginii principale și a paginii personale de rezervări, urmată de efectuarea unei căutări parametrizate pentru o proprietate.
\textbf{Scop:} Testează performanța operațiunilor de citire (read) din baza de date și capacitatea serverului de a genera pagini dinamice care implică interogări complexe.
\item \textbf{Fluxul Complet de Gestionare a Proprietăților (CRUD):}  
\textbf{Descriere:} Acoperă ciclul de viață complet al unei entități de tip „proprietate”, incluzând operațiuni de creare (Create), modificare (Update) și ștergere (Delete).  
\textbf{Scop:} Evaluează performanța operațiunilor de scriere (write) în baza de date și menținerea coerenței datelor în condiții de stres, testând în același timp tranzacțiile și mecanismele de blocare (locking).  

\item \textbf{Fluxul Tranzacțional End-to-End: Rezervare și Anulare:}  
\textbf{Descriere:} Simulează un flux complet de tranzacție, incluzând căutarea disponibilității, selectarea unei proprietăți, parcurgerea pașilor de confirmare a rezervării (cu multiple validări și scrieri în baza de date) și anularea aceleiași rezervări.  
\textbf{Scop:} Testează integritatea întregului sistem tranzacțional, de la interfața web până la logica de business și persistența datelor, asigurând că aplicația poate gestiona un volum mare de tranzacții concurente fără coruperea datelor.  

\item \textbf{Fluxul de Administrare a Contului de Utilizator:}  
\textbf{Descriere:} Simulează accesarea și modificarea datelor din profilul personal al utilizatorului.  
\textbf{Scop:} Evaluează performanța subsistemului de autentificare și autorizare, precum și a operațiunilor de actualizare a datelor specifice utilizatorului.  

\item \textbf{Fluxul de Solicitare a Resurselor Statice:}  
\textbf{Descriere:} Utilizatorul virtual solicită în mod repetat fișierele statice ale aplicației (CSS, JavaScript, imagini).  
\textbf{Scop:} Izolează și testează performanța serverului web (Nginx) în livrarea de conținut static, independent de serverul de aplicație, ajutând la identificarea eventualelor probleme de configurare sau limitări de lățime de bandă.  

\item \textbf{Simularea Căutărilor (Read-Heavy):}  
Scenariul a simulat utilizatori care efectuează căutări repetate de proprietăți, vizualizează detalii și navighează prin rezultate. A evaluat performanța operațiunilor de citire din baza de date, eficiența direcționării cererilor către nodul Slave și capacitatea Nginx de a gestiona un volum mare de cereri GET.  

\item \textbf{Simularea Înregistrărilor și Autentificărilor:}  
Acest scenariu a testat procesele de înregistrare a utilizatorilor noi și autentificare, implicând operațiuni de scriere (INSERT) și citire (SELECT), evaluând atât performanța nodului Master, cât și logica de sesiune gestionată de Flask.  

\item \textbf{Simularea Procesului de Rezervare (Write-Heavy/Transactional):}  
Cel mai complex scenariu, simulând întregul flux de rezervare de la căutare la finalizarea tranzacției. A implicat multiple operațiuni de scriere și actualizare (INSERT, UPDATE) și a evaluat performanța nodului Master și capacitatea sistemului de a gestiona tranzacții ACID complexe sub sarcină.  

\item \textbf{Testarea Rezilienței și Failover-ului:}  
Au fost simulate scenarii de eșec, cum ar fi oprirea unei instanțe Flask sau a MySQL Master, pentru a valida comportamentul de failover al Nginx și replicarea MySQL, precum și pentru a măsura timpul de recuperare a serviciului.  
...
\subsection{Cazul 1: Aplicația Flask Monolitică (Fără Scalare/Load Balancing)}
...
\begin{enumerate}
	...
	\begin{itemize}
\item A fost pornită o singură instanță a aplicației web Flask, rulând pe portul implicit 5000, responsabilă atât de logica funcțională, cât și de procesarea directă a cererilor HTTP.
\item Serverul principal MySQL, operativ pe sistemul gazdă, a gestionat toate tranzacțiile cu baza de date, incluzând atât operațiuni de citire, cât și de scriere.
\end{itemize}
	\item \textbf{Instanța Virtuală 1 (VM1):} A fost dezactivată pe durata testării, iar serverul MySQL secundar nu a fost utilizat.
\item \textbf{Instanța Virtuală 2 (VM2):} A fost dezactivată și nu a fost folosită, iar cealaltă instanță Flask nu a fost operativă.
\item \textbf{Nginx:} Componenta Nginx a fost dezactivată și nu a gestionat traficul. Cererile generate de Locust au fost direcționate direct către instanța Flask de pe sistemul gazdă, fără implicarea unui load balancer sau proxy invers.
\item \textbf{Depozitul de date:} Instanța principală MySQL de pe sistemul gazdă a fost conectată la aplicația Flask. Baza de date a fost populată cu un set de date de test (aproximativ 150 de utilizatori, 300 de proprietăți și 900 de unități locative), generat cu programul \texttt{generate\_test\_data.py}, asigurând uniformitate pentru operațiunile de citire și scriere pe durata testului.
\end{enumerate}
...
\section{Cazul 2: Aplicația Flask\textbf{Aplicația Flask } (Load Balancing)}
...
\section{Concluzii}
LLucrarea de licență a abordat studiul și implementarea unei aplicații web complexe, de tip sistem de rezervări de proprietăți, concentrându-se pe aspectele esențiale de arhitectură, implementare și scalabilitate. Prin analiza detaliată a fluxurilor funcționale principale și a mecanismelor de suport, s-a demonstrat o înțelegere aprofundată a provocărilor dezvoltării aplicațiilor distribuite și s-a realizat un sistem funcțional și robust.

Obiectivele principale ale lucrării au fost atinse cu succes:

\begin{itemize}
\item A fost proiectată și implementată o arhitectură \texttt{N-Tier} modulară, separând clar straturile de prezentare (\texttt{frontend}), logică de afaceri (\texttt{backend}) și date. Această abordare asigură mentenanță facilă, testabilitate și posibilități de scalare independentă a componentelor.
\item Au fost implementate fluxurile esențiale ale aplicației: înregistrarea și autentificarea utilizatorilor, căutarea și vizualizarea proprietăților, precum și procesul complex de creare a rezervărilor. S-a pus accent pe securitatea datelor sensibile, folosind mecanisme criptografice robuste, precum \texttt{bcrypt} pentru hashing-ul parolelor.
\item A fost dezvoltată o logică avansată pentru prevenirea \texttt{double booking}-ului, prin interogări SQL de suprapunere și gestionarea tranzacțională \texttt{ACID}, asigurând integritatea și consistența datelor în timpul operațiilor critice de rezervare.
\item S-au implementat fluxuri de management pentru proprietari (adăugare/editare proprietăți, vizualizare rezervări primite) și un panou de administrare cu control granular al accesului pe roluri (\texttt{session['is\_admin']}), esențial pentru supravegherea și controlul sistemului.
\item S-au integrat componente cheie de infrastructură, precum \texttt{Nginx} pentru \texttt{load balancing} și servirea fișierelor statice, și replicarea \texttt{MySQL Master-Slave} pentru înaltă disponibilitate și scalarea operațiilor de citire. Rolul infrastructurii de virtualizare a fost evidențiat în dezvoltarea și testarea acestor componente într-un mediu controlat.
\end{itemize}

Punctele forte ale arhitecturii și implementării includ:

\begin{itemize}
\item \textbf{Securitate robustă:} Implementarea \texttt{bcrypt} și a sesiunilor criptografice oferă protecție avansată a credențialelor și a integrității sesiunilor.
\item \textbf{Integritate tranzacțională:} Tranzacțiile \texttt{ACID} garantează că operațiile critice, precum rezervările, sunt sigure și consistente, prevenind stările de date incorecte.
\item \textbf{Scalabilitate și reziliență:} Prin utilizarea \texttt{Nginx} pentru \texttt{load balancing} și a replicării \texttt{MySQL}, aplicația poate gestiona volume mari de trafic și funcționează continuu chiar și în cazul unor defecțiuni.
\item \textbf{Modularitate:} Structura \texttt{N-Tier} și separarea responsabilităților contribuie la un cod ușor de înțeles, extins și depanat.
\item \textbf{Experiența utilizatorului:} Designul intuitiv și mecanismele de căutare „fuzzy” și recomandare a camerelor asigură o experiență pozitivă pentru utilizator.
\end{itemize}

Pe parcursul dezvoltării, au fost învățate lecții importante, precum complexitatea menținerii consistenței datelor în medii distribuite și necesitatea validării stricte a intrărilor de la utilizatori la nivel de backend. De asemenea, testarea exhaustivă a scenariilor de eșec s-a dovedit esențială pentru asigurarea robusteței sistemului.

\section{Direcții Viitoare de Dezvoltare}

Proiectul reprezintă o fundație solidă pentru o platformă de rezervări de proprietăți, cu multiple oportunități de extindere și îmbunătățire, menite să transforme aplicația într-o soluție completă și competitivă pe piață. Aceste direcții pot fi structurate pe două paliere principale:

\subsection{Extensii ale Funcționalităților Aplicației}
\begin{itemize}
\item \textbf{Sistem de Mesagerie Integrat:} Comunicare directă între chiriași și proprietari, cu notificări în timp real, pentru o experiență mai interactivă.
\item \textbf{Sistem de Recenzii și Rating-uri:} Permite utilizatorilor să evalueze proprietățile, construind încredere și oferind informații utile altor potențiali chiriași. Aceasta ar implica crearea unui tabel \texttt{reviews} și agregarea rating-urilor.
\item \textbf{Integrare cu Sisteme de Plăți Online:} Implementarea unui gateway de plată (ex: Stripe, PayPal) pentru finalizarea rezervărilor direct în platformă, cu gestionarea securizată a tranzacțiilor.
\item \textbf{Funcționalități Avansate de Căutare și Filtrare:} Adăugarea de filtre suplimentare și integrarea unei hărți interactive cu proprietăți pentru o navigare mai intuitivă.
\item \textbf{Notificări Automate:} Email-uri și mesaje \texttt{SMS} pentru confirmări, anulări, memento-uri sau mesaje noi, pentru creșterea angajamentului utilizatorilor.
\item \textbf{Gestionarea Disponibilității la Nivel de Calendar:} Calendar interactiv pentru proprietari și vizualizare clară a disponibilității pentru chiriași.
\end{itemize}

\subsection{Îmbunătățiri ale Performanței și Scalabilității Infrastructurii}
\begin{itemize}
\item \textbf{Introducerea unui Strat de Caching:} Utilizarea unui sistem de \texttt{caching} (ex: \texttt{Redis}, \texttt{Memcached}) pentru date frecvent accesate, reducând cererile către baza de date și scurtând timpii de răspuns.
\item \textbf{Arhitectura de Microservicii:} Refactorizarea aplicației pe servicii independente (autentificare, proprietăți, rezervări, plăți) pentru scalare și dezvoltare modulară.
\item \textbf{Servicii Cloud Gestiunate:} Migrarea bazei de date către un serviciu cloud gestionat (ex: Amazon RDS, Google Cloud SQL) și utilizarea load balancer-ilor cloud pentru scalare automată și \texttt{failover}.
\item \textbf{Content Delivery Network (CDN):} Servirea fișierelor statice prin CDN pentru reducerea latenței și îmbunătățirea vitezei de încărcare a paginilor la nivel geografic extins.
\end{itemize}
